...
extern "C" uint32_t asm_error(uint32_t a, uint32_t b); //extern function for part3
extern "C" uint32_t asm_add_6(uint32_t* int1, uint32_t*  int2, uint32_t*  int3, uint32_t* int4, uint32_t*  int5, uint32_t* int6); //extern function for extension part
...
void part2() {
	...
	"Whenever we want a particular vairable to retain it's value between function call, 
	here ledVar should retain its previous value, hence static is used."
	*/

	static uint8_t ledVar = 1;
	wait_us(10000);
...
void part3() {
	...
	static uint32_t val2 = 0x8FFFFFFF;  // set this to something that will cause an overflow
	
	// write your solution here:
	ovf = asm_error(val1, val2); //value of r0 is "1" now and is returned back to ovf variable.
	led2 = ovf; //led2 is 1 now. PG14 led will light up
...
void extension() {
	...
	result = asm_add_6(&a, &b, &c, &d, &e, &f); //parameters to the assembly function are the addresses of the integers
	// print the result
	printf("1+2+3+4+5+6 = : %ld\n\r", result);
}
